 ...
 */
public class aroopp.Genie.Parser : CodeVisitor {
	Scanner scanner;
	...
	void parse_class_member (Class cl) throws ParseError {
		...
		} else if (sym is aroopp.Signal) {
			cl.add_signal ((aroopp.Signal) sym);
		} else if (sym is Field) {
	...
	aroopp.Signal parse_signal_declaration (List<Attribute>? attrs) throws ParseError {
		var begin = get_location ();
		DataType type;
		...
		var sig = new aroopp.Signal (id, type, get_src (begin), comment);
		if (ModifierFlags.PRIVATE in flags) {
	...
	void parse_interface_member (Interface iface) throws ParseError {
		...
		if (sym is Class) {
			...
			iface.add_signal ((aroopp.Signal) sym);
		} else if (sym is Field) {
