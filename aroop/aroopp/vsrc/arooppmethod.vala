 ...
 */
public class aroopp.Method : Subroutine {
	Set<TypeParameter> type_parameters;
	...
	private Set<Parameter> parameters = new ArrayList<Parameter> ();
	private Set<Expression> preconditions;
	private Set<Expression> postconditions;
	...
	Set<LocalVariable> captured_variables;

	static Set<Expression> _empty_expression_list;
	...
	public Set<Parameter> get_parameters () {
		return parameters;
	}
	 ...
	 */
	public Set<TypeParameter> get_type_parameters () {
		if (type_parameters != null) {
	 ...
	 */
	public Set<Expression> get_preconditions () {
		if (preconditions != null) {
	 ...
	 */
	public Set<Expression> get_postconditions () {
		if (postconditions != null) {
	...
	public Set<Parameter> get_async_begin_parameters () {
		assert (this.coroutine);

		var aroopp_ns = CodeContext.get ().root.scope.lookup ("aroopp");
		...
		var callback_type = new DelegateType ((Delegate) aroopp_ns.scope.lookup ("AsyncReadyCallback"));
	...
	public Set<Parameter> get_async_end_parameters () {
		assert (this.coroutine);

		var params = new ArrayList<Parameter> ();

		var aroopp_ns = CodeContext.get ().root.scope.lookup ("aroopp");
		var result_type = new ObjectType ((ObjectTypeSymbol) aroopp_ns.scope.lookup ("AsyncResult"));
