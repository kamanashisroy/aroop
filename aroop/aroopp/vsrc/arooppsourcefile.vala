...
/**
 * Represents a aroopp source or VAPI package file.
 */
public class aroopp.SourceFile {
	/**
	...
	private ArrayList<Comment> comments = ArrayList<Comment> ();

	public Set<UsingDirective> current_using_directives { get; set; default = Set<UsingDirective> (); }

	private Set<CodeNode> nodes = Set<CodeNode> ();
	...
	private ArrayList<string> source_array = ArrayList<string>();
	...
	public void add_using_directive (UsingDirective ns) {
		...
		current_using_directives = Set<UsingDirective> ();
	...
	public string get_csource_filename () {
		if (csource_filename == null) {
			if (context.run_output) {
				...
				// temporary file
				csource_filename = Path.build_path ("/", get_destination_directory (), get_basename () + ".aroopp.c");
			}
		}
...
public enum aroopp.SourceFileType {
	NONE,
	SOURCE,
	PACKAGE,
	FAST
