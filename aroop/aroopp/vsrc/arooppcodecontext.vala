 ...
 */
public class aroopp.CodeContext {
	/**
	...
	/**
	 * Target major version number of aroopp for code generation.
	 */
	public int target_aroopp_major { get; set; }

	/**
	 * Target minor version number of aroopp for code generation.
	 */
	public int target_aroopp_minor { get; set; }
	...
	/**
	 * Returns true if the target version of aroopp is greater than or 
	 * equal to the specified version.
	 */
	public bool require_aroopp_version (int major, int minor) {
		return (target_aroopp_major > major) || (target_aroopp_major == major && target_aroopp_minor >= minor);
	}
	...
	public bool add_external_package (string pkg) {
		...
		if (path == null) {
			Report.error (null, "Package `%s' not found in specified aroopp API directories or GObject-Introspection GIR directories".printf (pkg));
			return false;
		}
	...
	/**
	 * Add the specified source file to the context. Only .aroopp, .vapi, .gs,
	 * and .c extensions are supported.
	 *
	 * @param filename a filename
	 * @param is_source true to force adding the file as .aroopp or .gs
	...
	public bool add_source_filename (string filename, bool is_source = false) {
		...
		if (is_source || filename.has_suffix (".aroopp") || filename.has_suffix (".gs")) {
			...
			if (profile == Profile.POSIX) {
				...
				// import the aroopp namespace by default (namespace of backend-specific standard library)
				var ns_ref = new UsingDirective (new UnresolvedSymbol (null, "aroopp", null));
				source_file.add_using_directive (ns_ref);
				root.add_using_directive (ns_ref);
				...
				source_file.add_using_directive (ns_ref);
				root.add_using_directive (ns_ref);
			} else if (profile == Profile.AROOP) {
				// import the Aroop namespace by default (namespace of backend-specific standard library)
				var ns_ref = new UsingDirective (new UnresolvedSymbol (null, "Aroop", null));
				source_file.add_using_directive (ns_ref);
				root.add_using_directive (ns_ref);
			}
			...
			Report.error (null, "%s is not a supported source file type. Only .aroopp, .vapi, .gs, and .c files are supported.".printf (filename));
			return false;
		}
	...
	public string? get_vapi_path (string pkg) {
		var path = get_file_path (pkg + ".vapi", "aroopp" + Config.PACKAGE_SUFFIX + "/vapi", "aroopp/vapi", vapi_directories);
