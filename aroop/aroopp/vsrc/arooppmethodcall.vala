 ...
 */
public class aroopp.MethodCall : Expression {
	/**
	 * The method to call.
	 */
	...
	public override bool is_constant () {
		...
		if (method_type != null) {
			...
			if (method_type.method_symbol.get_full_name () == "aroopp.N_") {
				// first argument is string
				return argument_list[0].is_constant ();
			} else if (method_type.method_symbol.get_full_name () == "aroopp.NC_") {
		...
		if (mtype is ObjectType || (context.profile == Profile.GOBJECT && call.symbol_reference == context.analyzer.object_type)) {
			...
			if (mtype is ObjectType) {
				...
				if (cl == null || !cl.is_subtype_of (context.analyzer.object_type)) {
					error = true;
					Report.error (source_reference, "chain up to `aroopp.Object' not supported");
					return false;
				}
