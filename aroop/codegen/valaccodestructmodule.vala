	...
	public override void generate_struct_declaration (Struct st, CCodeFile decl_space) {
		...
		foreach (Field f in st.get_fields ()) {
			...
			if (f.binding == MemberBinding.INSTANCE)  {
				...
				instance_struct.add_field (field_ctype, get_ccode_name (f) + get_ccode_declarator_suffix (f.variable_type), f.deprecated ? " G_GNUC_DEPRECATED" : null, generate_declarator_suffix_cexpr(f.variable_type));
