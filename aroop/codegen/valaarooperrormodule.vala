...
public class Vala.AroopErrorModule : AroopDelegateModule {
	...
	public override void generate_error_domain_declaration (ErrorDomain edomain, CCodeFile decl_space) {
		if (add_symbol_declaration (decl_space, edomain, get_error_module_lower_case_name (edomain))) {
			return;
		}

		// provide error listing for the domain
		var cenum = new CCodeEnum (get_error_module_lower_case_name (edomain));

		foreach (ErrorCode ecode in edomain.get_codes ()) {
			if (ecode.value == null) {
				cenum.add_value (new CCodeEnumValue (get_error_module_lower_case_name (ecode)));
			} else {
				ecode.value.emit (this);
				cenum.add_value (new CCodeEnumValue (get_error_module_lower_case_name (ecode), get_cvalue (ecode.value)));
			}
		}
	...
	public override void visit_error_domain (ErrorDomain edomain) {
		...
		push_function (cdomain_desc);
		// add description
		ccode.add_return (new CCodeConstant ("\"" + get_ccode_lower_case_name (edomain) + "-domain\""));
		pop_function ();
		cfile.add_function (cdomain_desc);
	}

	public override void visit_throw_statement (ThrowStatement stmt) {
		var throw_exception = new CCodeFunctionCall(new CCodeIdentifier ("aroop_throw_exception"));
		throw_exception.add_argument(get_cvalue(stmt.error_expression));
...
#endif		
		ccode.add_expression (throw_exception);
	...
	public override void visit_catch_clause (CatchClause clause) {
		...
		if (clause.error_variable != null) {
			...
			ccode.add_assignment (get_variable_cexpression (get_variable_cname (clause.error_variable.name)), get_variable_cexpression ("_inner_error_"));
		}
#endif
