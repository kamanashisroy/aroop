...
public class Vala.AroopCCodegenImpl : AroopErrorModule {
	...
	public override void add_simple_check (CodeNode node, bool always_fails = false) {
		if (always_fails) {
			...
			var ccond = new CCodeBinaryExpression (CCodeBinaryOperator.INEQUALITY, new CCodeIdentifier ("aroop_error"), new CCodeConstant ("NULL"));
			ccode.open_if (ccond);
		}
		...
		if (current_try != null) {
			...
			if (!is_in_catch) {
				...
				foreach (CatchClause clause in current_try.get_catch_clauses ()) {
					...
					if (clause.error_type.equals (new ObjectType (error_class))) {
						...
						type_check.add_argument (new CCodeIdentifier ("aroop_error"));
			...
			foreach (DataType error_type in current_method.get_error_types ()) {
				// If Aroop.Error is allowed we propagate everything
	...
	public override void visit_catch_clause (CatchClause clause) {
		...
		if (clause.variable_name != null) {
			...
			cdecl.add_declarator (new CCodeVariableDeclarator (variable_name, new CCodeIdentifier ("aroop_error")));
			ccode.add_statement (cdecl);
		} else {
			// error object is not used within catch statement, clear it
			var cclear = new CCodeFunctionCall (new CCodeIdentifier ("aroop_object_unref"));
			cclear.add_argument (new CCodeIdentifier ("aroop_error"));
			ccode.add_statement (new CCodeExpressionStatement (cclear));
		}
		ccode.add_statement (new CCodeExpressionStatement (new CCodeAssignment (new CCodeIdentifier ("aroop_error"), new CCodeConstant ("NULL"))));
